...
class TrainImage():
    ...
    def load_mask(self):
        ...
        for i, c in enumerate(self.labels):
            for poly in self.polygons[c]:
                if len(poly) == 1:
                    ...
                    # add middle point to self.sample_positions
                    avg_y = int(np.mean([p[1] for p in poly[0]]))
                    avg_x = int(np.mean([p[0] for p in poly[0]]))
                    sample_positions.append((avg_x, avg_y))
                elif len(poly) > 1:
                    for polym in poly:
        ...
        for region in self.regions:
            ...
            region["sample_positions"] = [s for s in sample_positions if region['x'] <= s[0] <= region['x'] + region['w'] and region['y'] <= s[1] <= region['y'] + region['h']]
