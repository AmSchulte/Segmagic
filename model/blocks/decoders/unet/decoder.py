...
class UnetDecoder(nn.Module):
    ...
    Supports optional deep supervision where intermediate decoder outputs can be used for auxiliary losses.
    """
    ...
    def forward(self, features: List[torch.Tensor]) -> Union[torch.Tensor, List[torch.Tensor]]:
        ...
        for i, decoder_block in enumerate(self.blocks):
            ...
            # Store intermediate outputs for deep supervision (excluding the final output)
            if self.deep_supervision and i < len(self.blocks) - 1:
                intermediate_outputs.append(x)

        if self.deep_supervision:
            return [x] + intermediate_outputs  # Main output first, then intermediate outputs
        else:
            return x